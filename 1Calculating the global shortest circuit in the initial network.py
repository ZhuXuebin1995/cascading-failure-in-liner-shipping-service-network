...
def max_interval_load(aim_interval_dict,max_load):#max_load = [],0装,1卸，2装船期，3卸船期
    for m in aim_interval_dict:
        for d in aim_interval_dict[m]:
            for t in aim_interval_dict[m][d]:
                total_load = aim_interval_dict[m][d][t][1] - aim_interval_dict[m][d][t][0]
                if total_load > max_load[1] - max_load[0]:
                    max_load[0] = aim_interval_dict[m][d][t][0]
                    max_load[2] = aim_interval_dict[m][d][t][2]
                    max_load[1] = aim_interval_dict[m][d][t][1]
                    max_load[3] = aim_interval_dict[m][d][t][3]
    return max_load

def count_load(load_dicts,max_load_dict,port_interval_tree):#time_unit是累加的时间单元，默认为半小时
        ...
        for t in load_dicts[p].keys():
            ...
            if load_dicts[p][t][2] < 0 :
                while t_count <= t[1] + time_diff:
                    ...
                    times_can[month_num][date_num][hour_num][2].append(t[0])
                ...
                while t_count >= t[1] - time_diff:
                    ...
                    times_can[month_num][date_num][hour_num][3].append(t[0])
                    t_count -= datetime.timedelta(minutes=30)
        max_load = [0,0,0,0]#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        max_load_dict[p] = copy.deepcopy(max_interval_load(times_can,max_load))#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        max_load.clear()
...
port_max_load = {}
port_interval_dict = {}
count_load(load_dict,port_max_load,port_interval_dict)
build_file("port max load.json",port_max_load)
print("Finish port's max load count！")

#生成中转次数的表
# for i in t_dict.keys():
#     t_dict[i] = str(t_dict[i])
# df = pd.DataFrame.from_dict(t_dict,orient='index',columns=['transport times'])
# df = df.reset_index().rename(columns = {'index':'od pair'})
# writer = pd.ExcelWriter(r'D:\Users\朱先森\Desktop\t_dict2.xlsx')
# df.to_excel(r'D:\Users\朱先森\Desktop\t_dict2.xlsx')
# print("Finish transport times excel!")

# #生成旅行时间的json数据
# json_str1 = json.dumps(travel_time_dict)
# file_name1 = "travel time-after 0.json"
# with open(file_name1,'w') as json_object:
#     json_object.write(json_str1)
# print("Finish transport time json file")
# #生成节点负荷
#

# json_str = json.dumps(port_max_load)
# file_name = "port's max load 27.json"
# with open(file_name,'w') as json_file:
#     json_file.write(json_str)
#
# print("Finish port's max load json file!")
...
new_whole_od = trans_whole_od(whole_od_dict)
new_key_path = change_dict(new_whole_od)

#此时输出的路径字典的键已经转变成数字，在下一次程序载入时，从值中恢复od点对

build_file("whole od path (key as number).json",new_key_path)
